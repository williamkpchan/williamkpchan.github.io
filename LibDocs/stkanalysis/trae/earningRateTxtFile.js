...
const { TextDecoder } = require('util'); // For GB18030 decoding
...
async function fetchStockData() {
  ...
  for (const code of codeTableH) {
      ...
      stocks.push({
        code: code,
        name: resultarr[1] || 'N/A',
        pe: parseFloat(resultarr[39]) || 0
      });
  }
  ...
  // 3. Generate output
  let outputContent = 'Stock Code | Stock Name          | P/E Ratio\n';
  stocks.forEach(stock => {
    outputContent += `${stock.code } ${stock.name } ${stock.pe.toFixed(2)}\n`;
  });
