...
const codeTableH = ['00388', '02208'];
const urlheader = "https://qt.gtimg.cn/?q=";
...
async function fetchStockData() {
  ...
  for (const code of codeTableH) {
      url = `https://www.aastocks.com/tc/stocks/quote/quick-quote.aspx?symbol=${code}`;
console.log(url)
...
console.log($)
        ...
        const stockName = $('#SQ_Name').text();
console.log("stockName ",stockName)
        const rowText = $('#tbQuote > tbody > tr:nth-child(5) > td:nth-child(2) > div.ss2.abs.lbl_r.font-num.cls').text().trim() || 'Row not found';
console.log("rowText ",rowText)
        
        stocks.push({
          code: code,
          name: stockName,
          rowText: parseFloat(rowText),
        });
  }

  // 2. Sorting options (uncomment your preferred method)
  // Sort by P/E ratio (ascending):
  stocks.sort((a, b) => a.rowText - b.rowText);
  
  // Sort by stock name (A-Z):
  // stocks.sort((a, b) => a.name.localeCompare(b.name, 'zh-Hans-CN'));
  
  // Sort by stock code:
  // stocks.sort((a, b) => a.code.localeCompare(b.code));

  // 3. Generate output
  let outputContent = 'Stock Code | Stock Name          | Earnings\n';
  stocks.forEach(stock => {
    outputContent += `${stock.code } ${stock.name } ${stock.rowText.toFixed(2)}\n`;
  });

  // 4. Save to file
  await fs.writeFile('sorted_stocks.txt', outputContent);
  console.log('Success! Sorted results saved to sorted_stocks.txt');
}
