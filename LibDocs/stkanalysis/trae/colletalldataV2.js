	...
	try {
		...
		rows.forEach(row => {
			...
			if (columns.length > 1) {
				...
				if (!codeTable.includes(stknum)) {
					console.warn(`Stock code ${stknum} not in codeTable`);
					return;
				}

				if (!BaseObj[stknum]) {
					console.warn(`No BaseObj data found for stock code: ${stknum}. Initial fetch may have failed.`);
					return;
				}
