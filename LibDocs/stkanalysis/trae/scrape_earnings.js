//https://www.aastocks.com/en/stocks/quote/quick-quote.aspx?symbol=01800
//<label id="SQ_Name" title="中國交通建設">中國交通建設</label> 
//#tbQuote > tbody > tr:nth-child(5)

const puppeteer = require('puppeteer');
const fs = require('fs');

// The array of stock codes
...
(async () => {
  try {
    ...
    // Function to scrape a single stock code
    const scrapeStock = async (code) => {
      try {
        // Navigate to stock page
        const url = `https://www.aastocks.com/tc/stocks/quote/quick-quote.aspx?symbol=${code}`;
        await page.goto(url, { waitUntil: 'networkidle2', timeout: 30000 });
        
        // Extract stock name and row text
        const stockName = await page.$eval('#SQ_Name', el => el.textContent.trim())
          .catch(() => 'Name not found');
          
        const rowText = await page.$eval('#tbQuote > tbody > tr:nth-child(5)', el => el.textContent.trim())
          .catch(() => 'Row not found');
        ...
        console.log(`Successfully scraped: ${code} - ${stockName}`);
      } catch (error) {
        console.error(`Error scraping ${code}:`, error.message);
        results.push({
          code,
          name: 'Scraping error',
          rowText: error.message
        });
      }
    };
    
    // Scrape each stock code (with delay to avoid blocking)
    for (const code of codeTableH) {
      await scrapeStock(code);
    ...
    let fileContent = "Stock Code,Stock Name,Row Text\n"; // CSV header
    results.forEach(item => {
      // Escape commas in text by enclosing in quotes
      const escapedName = item.name.includes(',') ? `"${item.name}"` : item.name;
      const escapedRowText = item.rowText.includes(',') ? `"${item.rowText}"` : item.rowText;
      fileContent += `${item.code},${escapedName},${escapedRowText}\n`;
    });
    
    // Write to file
    fs.writeFileSync('stock_results.txt', fileContent, 'utf8');
    console.log(`Results saved to 'stock_results.txt' (${results.length} entries)`);
