...
// Loop through each code in the array
for (let i = 0; i < codeTableH.length; i++) {
    // Construct the URL
    const url = urlheader + "hk" + codeTableH[i];
    
    // Fetch the data (note: fetch returns a promise)
    fetch(url)
        .then(response => {
            if (!response.ok) {
        ...
        .then(result => {
            ...
            // Log the 40th element (index 40)
            console.log(`${codeTableH[i]} 市盈率: ${resultarr[39]}`);
        })
        .catch(error => {
            console.error(`Error processing ${codeTableH[i]}:`, error);
        });
}
