...
async function fetchKline(theCode) {
    ...
    .then(text => {
      ...
      dataObj = stkdata.data[`${theModCode}`].qfqday || stkdata.data[`${theModCode}`].day

      dateArr = []
      closeArr = [];

      for (let i = 0; i < dataObj.length; i++) {
        dateArr.push(dataObj[i][0]);  // no Number here
...
for (let i = 0; i < codeTable.length; i++) {
  checkCodeLen(codeTable[i])
  fetchKline(theModCode)
}
