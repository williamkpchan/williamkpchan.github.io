...
//const codeTable = ['00388', '00002', '00003', '00005', '00006', '00011', '00012'];
...
function main() {
  (async () => {
    ...
    arrLength = BaseObj["00388"].closeArr.length // this will decide plot length
...
console.log("arrLength ", arrLength)
      ...
      for (let day = (arrLength-29); day < arrLength; day++) {
        highpassCnt=0
        highFailCnt=0
        lowpassCnt=0
        lowFailCnt=0
      ...
      // append statusRpt
      codeLen = codeTable.length
      lastIdx = highpassCntArr.length-1
      $("#statusRpt").append(
       "wma3Up: <r>" + wma3UpCnt + "</r>" +
       " wma3Dn: <lg>" + wma3DnCnt + "</lg> " +
       " wma3Up%: <r>" + Math.round(wma3UpCnt*100/codeLen) + "</r>" +
       " wma3Dn%: <lg>" + Math.round(wma3DnCnt*100/codeLen) + "</lg><br>" +
       "CAboveWma3: <r>" + closepassCntArr[lastIdx] + "</r>" +
       " CBelowWma3: <lg>" + closeFailCntArr[lastIdx] + "</lg>" +
       " CAboveWma3%: <r>" + Math.round(closepassCntArr[lastIdx]*100/codeLen) + "</r>" +
       " CBelowWma3%: <lg>" + Math.round(closeFailCntArr[lastIdx]*100/codeLen) + "</lg>"
       )
...
function showStat() {
console.log("highpassCntArr ",highpassCntArr.length, lowpassCntArr.length, closepassCntArr.length)
  plotArrays("高过三日中线",[highpassCntArr, lowpassCntArr, closepassCntArr], ['Hpass', 'Lpass','Cpass'])
  plotArrays("低过三日中线",[highFailCntArr, lowFailCntArr, closeFailCntArr], ['HFail', 'LFail','CFail'])

  plotArrays("多种中线升",[status3UpCntArr, status4UpCntArr, status5UpCntArr, status10UpCntArr], ['3Up','4Up','5Up','10Up'])
  plotArrays("多种中线跌",[status3DnCntArr, status4DnCntArr, status5DnCntArr, status10DnCntArr], ['3Dn','4Dn','5Dn','10Dn'])

  plotArrays("多种中线上突破",[statusUpXCnt3Arr, statusUpXCnt4Arr, statusUpXCnt5Arr, statusUpXCnt10Arr],['UpX3','UpX4','UpX5','UpX10'])
  plotArrays("多种中线下突破",[statusDnXCnt3Arr, statusDnXCnt4Arr, statusDnXCnt5Arr, statusDnXCnt10Arr],['DnX3','DnX4','DnX5','DnX10'])
		...
		fetch(theurl)
			...
			.then(text => {
				...
				if (stkdata.code === 0) {
					...
					if(theCode=="00388"){ // prepare the date array
						for (let i = 21; i < newDataObj.length; i++) {
							dateArr.push(newDataObj[i][0]);
						} 
					}
...
function compareHLC(curHigh, curLow, curClose, closeArr ) {
	curWma3 = calculateWeightedMovingAverage(closeArr, 3); // calc 3 days
     elements = closeArr.slice(-3-1, -1); // period of 3
	prevWma3 = calculateWeightedMovingAverage(elements, 3); // calc 3 days
	if(curWma3 > prevWma3){
		wma3UpCnt = wma3UpCnt + 1
	}
	if(curWma3 < prevWma3){
		wma3DnCnt = wma3DnCnt + 1
	}
...
function plotArrays(title,arrays, labels) {
  ...
  // Generate datasets, it is an object
