...
function showStat(){
    h2content = document.createElement("h3");
    statMsg = showDate() +" "+ showTime() + "<br>" + "趋势统计表 总数："+ codeTable.length +  "<br>"
    statMsg = statMsg + " <r>3日趋势升：" +status3UpCnt + "</r> "
    statMsg = statMsg + " <gr>3日趋势跌：" +status3DnCnt + "</gr>" + "<br>"

    statMsg = statMsg + " <r>10日趋势升：" +status10UpCnt + "</r> "
    statMsg = statMsg + " <gr>10日趋势跌：" +status10DnCnt + "</gr>" + "<br>"

    h2content.innerHTML = statMsg
    document.body.appendChild(h2content);
