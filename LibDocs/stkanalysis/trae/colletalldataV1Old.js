...
async function fetchKline(theCode, theurl) {
    try {
        ...
        if (stkdata.data && stkdata.data[theCode]) {
            ...
            console.log(`Request failed for ${theCode}: ${stkdata.msg || 'No data available'}`);
            return {
                code: theCode,
                stkname: "N/A",
                data: null,
                error: stkdata.msg || 'No data available'
            };
        }
    } catch (error) {
        console.log(`ERROR! ${theCode}: ${error}`);
        return {
            code: theCode,
            stkname: "N/A",
            data: null,
...
async function fetchAllData() {
    for (let i = 0; i < codeTable.length; i++) {
        ...
        //console.log(`Fetching data for ${modifiedCode}...`);
        ...
        try {
            ...
            console.error(`Failed to fetch data for ${modifiedCode}:`, error);
            allStocksData.push({
                code: modifiedCode,
                stkname: "N/A",
                data: null,
    ...
    console.log("All data fetched:", allStocksData);
    return allStocksData;
}

// Start the process
fetchAllData().then(() => {
    // Do something with allStocksData here
    console.log(allStocksData[0].code)
    console.log(allStocksData[0].stkname)
    console.log(allStocksData[0].data.dates)
    console.log(allStocksData[0].data.closes)
});
