...
async function collectdata(stknum) {
	...
	const url = "https://web.ifzq.gtimg.cn/appstock/app/hkfqkline/get?_var=kline_dayqfq&param=" + stkcode + ",day,,,16,qfq";
...
function checkAllTrends(stkNum) {
  const curObj = BaseObj[stkNum];
  const closeArr = curObj.closeArray;
  const stkname = curObj.stockName;

  if (closeArr.length < 2) return;
  ...
  const currentWMAs = periods.map(period => 
    calculateWeightedMovingAverage(closeArr, period)
  );
  ...
  const selectiveIncreasingWMAs = comparisonPairs.every(pair => {
    const shortIndex = periods.indexOf(pair.short);
    const longIndex = periods.indexOf(pair.long);
    
    // Ensure both periods exist in the array
    if (shortIndex === -1 || longIndex === -1) return false;

    return currentWMAs[shortIndex] < currentWMAs[longIndex];
  });

  // Check if current WMAs > previous WMAs (all periods)
  const prevWMAs = periods.map(period => 
    calculateWeightedMovingAverage(closeArr.slice(0, -1), period)
  );
  const allTrendsUp = currentWMAs.every((wma, index) => 
    wma > prevWMAs[index]
  );
...
async function updateChanges() {
	...
	allUpList =[]   // fetchDataChunks in updateInfo will evaluate results
     allUpCnt = 0
	await updateInfo()
}
...
async function updateInfo() {
	...
	showLargeAmtTable()
     ...
     allUpList.push("Total: " + allUpCnt + "<br>")
	document.getElementById('allTrendUpNote').innerHTML = allUpList.join("");
